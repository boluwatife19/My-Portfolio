...
export function ButtonOne({ to, children }) {
  return (
    <a href={to} className="relative px-5 py-2 font-medium text-white group">
...
export function ButtonTwo({ to, children }) {
  ...
  return (
        <Link
          ...
          className="rounded-md px-3.5 py-2 overflow-hidden relative group cursor-pointer border-2 font-medium border-sky-500 text-indigo-600 text-white w-24 sssxl:w-32 md:w-24 lg:w-32"
        >
          ...
          <span className="relative text-sky-500 transition duration-300 group-hover:text-white ease flex items-center ease">
            {children}
          </span>
        </Link>
  );
